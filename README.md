[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/pgqjtOmP)
[![Open in Visual Studio Code](https://classroom.github.com/assets/open-in-vscode-2e0aaae1b6195c2367325f4f02e2d04e9abb55f0b24a779b69b11b9e10269abc.svg)](https://classroom.github.com/online_ide?assignment_repo_id=18278583&assignment_repo_type=AssignmentRepo)
...
3) Use the following team naming format when you create your team on GitHub: spring25-lab1-GitHubid1-Githubid2. For example, spring25-lab1-alice-bob for a group of two. For a single group team, use spring25-lab1-alice as an example team name for a student with github id alice. If you already chose a different format for your team, edit your team name to the above format. 
...
The server hosts the world's smallest stock market that traders only two meme stocks: (i) GameStart and (ii) RottenFishCo,
...
- You should also implement your own synchronization code (using locks, semaphore, or other suitable primitive)
  to ensure thread safety when adding or removing requests from the request queue. For example, you are not allowed  to use a thread-safe queue library, which does this synchronization automatically for you.
...
stocks. In addition to GameStart and RottenFishCo, the catalog should also include BoarCo (for Boar
...
maintained in memory and __no database backend is needed for this lab.__ Implement appropriate
...
running on a different container from the server code, which runs in its own docker container. While it is fine
to run both client and server containers on the same machine, as an additional optional part, you can run the server container on one machine and the client container(s) on another (e.g., you lab partner's machine). However, for lab 1, it is sufficient to run the client and server code in containars running the same machine and it is not necessary to run them on different machines. Measure the latency seen
by the client for different types of requests, such as lookup and trade.

Vary the number of clients from 1 to 5 and measure the latency as the load goes up. To do this, you can start more than client in the same client container or run each client in different container - this is up to you. Make simple
...
2) Submit the following additional documents inside the `docs` directory. 1) A Brief design document
   (2 to 3 pages) that explains your interfaces, design choices (include citations, if you used referred to
...
4) Your GitHub repo is expected to contain **many commits with proper commit messages** (which is good
   ...
   git for code development as one should).

Notes:
 -  Since this is a graduate class, we assume you know how to write a good design doc. Writing proper technical descriptions to discuss the details of your system is an important aspect of designing systems and  communicating your design choices to others. To see some examples, please refer to https://www.designdocs.dev/library
...
and should be submitted through pizza (use exceptionRequests folder in pizza). A total of four free late days
